...
interface PurchaseRepository : JpaRepository<Purchase, Long> {
    ...
    @Query("SELECT DISTINCT p.stockLocation FROM Purchase p WHERE p.country = :country AND (p.pol IS NULL OR TRIM(p.pol) = '') AND p.stockLocation IS NOT NULL AND TRIM(p.stockLocation) != '' ORDER BY p.stockLocation")
    ...
    @Query("SELECT DISTINCT p.chassis FROM Purchase p WHERE p.country = :country AND p.stockLocation IN :stockLocations AND (p.pol IS NULL OR TRIM(p.pol) = '') AND (p.shipped IS NULL OR p.shipped = false) AND p.chassis IS NOT NULL AND p.chassis != '' ORDER BY p.chassis")
    fun findFilteredChassisByCountryAndLegacyStockLocations(
        @Param("country") country: String,
        @Param("stockLocations") stockLocations: List<String>
    ): List<String>

    @Query("SELECT p FROM Purchase p WHERE p.country = :country AND p.stockLocation IN :stockLocations AND (p.pol IS NULL OR TRIM(p.pol) = '') AND (p.shipped IS NULL OR p.shipped = false) ORDER BY p.chassis")
